...
%same formula
%coefficients_est =  sinh(-i*K*pi) ./(pi^2 *K*-i);
coefficients_est =  sin(K*pi) ./(pi^2 *K);
...
region1_1 = (domain < jump(1));
region1_2 =  (domain > jump(1));

region2_1 = (domain < jump(2));
region2_2 = (domain > jump(2));
...
for x = 1:size(jump_sum)
    for j = 1:JUMPS
        jump_sum(x) = jump_sum(x) + (r_jump(j) * g(j,x));
    end
end
